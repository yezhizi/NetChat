...
template <typename V> class SArray {
  public:
    // for string support
    SArray(const std::string &val)
        : SArray((char *)val.c_str(), (size_t)val.size()) {}
     ...
     */
    template <typename W> explicit SArray(const SArray<W> &arr) { *this = arr; }
     ...
     */
    template <typename Deleter> void reset(V *data, size_t size, Deleter del) {
        size_ = size;
        capacity_ = size;
        ptr_.reset(data, del);
    }
